...
def optimization_thread(url_list, device, loss_fkt, logger, args, target_hashes, bin_hex_hash_dict, target_hash_dict, model_path, epsilon):
    ...
    while (url_list != []):
        ...
        for i in tqdm(range(10000)):
            ...
            # # #todo L1 or L_inf
            # l1_norm = torch.norm(delta, p=1)
            # print(l1_norm)
            # if l1_norm > epsilon:
            #     delta.data = epsilon * delta.data.sign() / l1_norm
                # delta.grad.zero_().detach()

            # delta.data.clamp_(-epsilon, epsilon) #l_inf norm
            delta.data = (delta + delta.grad.detach().sign()).clamp(-epsilon, epsilon)
            # with torch.no_grad():
