...
class Linf_Norm:
    """Applies L_inf norm perturbation with given epsilon."""

    def __init__(self, epsilon):
        self.epsilon = epsilon

    def __call__(self, x):
        perturbation = torch.empty_like(x).uniform_(-self.epsilon, self.epsilon)
        return torch.clamp(x + perturbation, 0, 1)


class L2_Norm:
    """Applies L2 norm perturbation with given epsilon."""
...
class L1_Norm:
    """Applies L1 norm perturbation with given epsilon."""
    ...
    def __call__(self, x):
        perturbation = torch.randn_like(x)
        perturbation = perturbation / torch.norm(perturbation, p=1)
        perturbation = perturbation * self.epsilon
        return torch.clamp(x + perturbation, 0, 1)
