    ...
    for filename in os.listdir(reference_folder):
        ...
        if filename.startswith('test'):
            ...
            if filename.endswith('.pkl'):
                reference_log_files.append(filename)
                source_log_files.append(f"{filename.split('.')[0]}.pkl") # all new source should be in pkl format
    ...
    for s, r in zip(source_log_files, reference_log_files):
        ...
        for key in rout.keys() & sout.keys():
            ...
            if isinstance(svalue, torch.Tensor):
                ...
                if not ignore_visited_neurons and not (svalue is None or rvalue is None) and not abs(svalue - rvalue) / (rvalue + 1e-8) < 0.2:
                    warn_msg.append(f'{key} warning: {svalue} <- {rvalue}')
                    result = Result.Warning
            elif key == 'results':
