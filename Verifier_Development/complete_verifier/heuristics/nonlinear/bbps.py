...
class NonlinearBranching(NeuronBranchingHeuristic):
    ...
    def _get_optimized_branching_points(self, node, domains, branching_point_method):
        ...
        if isinstance(output_node, eval(self.branched_node)):
            ...
            elif len(self.net.net[node.output_name[0]].inputs) == 2:
                other_input = [
                    n for n in self.net.net[node.output_name[0]].inputs
                    if n.name != node.name][0]
                inp_idx = self.net.net[node.output_name[0]].inputs.index(node)
                lb_ori_other = domains['lower_bounds'][other_input.name].mean(dim=-1, keepdim=True)
                ub_ori_other = domains['upper_bounds'][other_input.name].mean(dim=-1, keepdim=True)
                if lb_ori.shape[3] == lb_ori_other.shape[2]:
                    lb_ori_other = lb_ori_other.transpose(-2, -1).expand(*lb_ori.shape)
                    ub_ori_other = ub_ori_other.transpose(-2, -1).expand(*lb_ori.shape)
                elif lb_ori.shape[2] == lb_ori_other.shape[2]:
                    lb_ori_other = lb_ori_other.expand(*lb_ori.shape)
                    ub_ori_other = ub_ori_other.expand(*lb_ori.shape)
                else:
                    raise ValueError
