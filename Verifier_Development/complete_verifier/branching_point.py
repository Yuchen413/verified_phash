...
def get_loss(node, mask, lower_branched, upper_branched):
    inputs = []
...
def optimize_points(lower, upper, mask):
    param = torch.nn.Parameter(
        torch.ones(lower[0].shape[0], args.num_branches, device=args.device)
        * (1. / args.num_branches))
    opt = torch.optim.Adam([param], lr=args.lr)

    loss_best = None
    points_best = None

    node = eval(args.op)(attr={'device': args.device}).to(args.device)
    ...
    for bp_idx in range(args.num_inputs):
        for t in range(args.num_iterations):
            ...
            loss = get_loss(node, mask, lower_branched, upper_branched)
