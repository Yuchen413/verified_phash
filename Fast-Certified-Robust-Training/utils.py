...
def prepare_model(args, logger, config):
    ...
    #     model_ori.load_state_dict(torch.load('models/new_model.pth'))

    if config['data'] == 'NH':
        onnx_model = onnx.load('/home/yuchen/code/verified_phash/Verifier_Development/complete_verifier/hash_360.onnx')
        model_ori = eval(model)(onnx_model, **parse_opts(args.model_params))

    if config['data'] == 'coco':
        model_ori = eval(model)(**parse_opts(args.model_params))
        # model_ori.load_state_dict(torch.load('/home/yuchen/code/verified_phash/Normal-Training/coco-hash-model.pt'))
