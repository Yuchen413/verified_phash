...
#sys.path.append('/content/drive/MyDrive/Verifier_Development-master/')
...
def Train(model, model_ori, t, loader, eps_scheduler, opt, loss_fusion=False, valid=False):
    ...
    for i, (data, labels) in enumerate(loader):
        ...
        for k in range(grad_acc):
            ...
            regular_loss, robust_loss, regular_err, robust_err = cert(
            ...
            update_meter(meter, regular_loss, robust_loss, regular_err, robust_err, data.size(0))
            if reg:
        ...
        for t in range(epoch + 1, args.num_epochs + 1):
            ...
            if t % args.test_interval == 0:
                ...
                if eps_scheduler.get_eps() == eps_scheduler.get_max_eps():
                    if args.natural:
                        if meter.avg('Err') < best[1]:
                            is_best, best = True, (meter.avg('Err'), t)
                        logger.info('Best epoch {}, error {:.4f}'.format(
                            best[-1], best[0]))
                    else:
